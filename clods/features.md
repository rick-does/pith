...
- **Project menu structure:** Projects (New project, Open project…, Project info, last-5-recents+archive) · File (New file, Add File from Markdown) · YAML (Quick open YAML…, View YAML) · Flatten/Restore hierarchy · Template (View template, Compliance) · Validate links · View HTML/PDF · Scan Project · Import from... · Export to... · Settings
...
- **Quick Open YAML (`POST /api/quick-open/yaml`):** YAML flyout → browse to any `.yaml`/`.yml` file; creates a ghost project with `tree_yaml` pointing at the selected file; `markdowns_dir` inferred as sibling `markdowns/` dir if it exists, else the YAML file's parent; deduplicates — if a project already has that `tree_yaml` path, reopens it instead of creating a new one
- **Drag-drop markdown:** dropping a `.md` file onto PiTH opens the Add File from Markdown dialog; `.yaml`/`.yml` drops show an alert directing to YAML → Quick open YAML…; drag-drop cannot provide a file path in browser mode, so no direct reference is possible on drop
