...
- **New Project dialog:** fields: title, project name (slugified from title), markdowns directory (required — where new files are created), YAML file (optional — browse to any `.yaml`/`.yml` file); markdowns dir is always required even when pointing to a YAML; if YAML is selected first, markdowns dir auto-suggests `<yaml-dir>/markdowns/` in muted style with a note; markdown browser opens at yaml's parent when yaml is set; YAML browser shows `.yaml`/`.yml` files as clickable-to-select (dirs still navigable); backend receives `{markdowns_dir, tree_yaml}` on `POST /api/projects/{name}`; no "New Project from Markdowns" copy flow — users point at existing dirs instead
