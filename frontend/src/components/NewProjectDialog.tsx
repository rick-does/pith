...
export default function NewProjectDialog({ currentProject, onCreated, onClose }: Props) {
  ...
  const openBrowser = useCallback(async (target: BrowserTarget) => {
    ...
    if (target === "markdowns") {
      if (yamlFile) {
        startPath = parentDir(yamlFile);
      } else {
        startPath = await browseStartDir(currentProject ?? undefined).catch(() => "");
      }
    } else {
      startPath = mdPath ? parentDir(mdPath) : await browseStartDir(currentProject ?? undefined).catch(() => "");
    }
    await navigate(startPath, target === "yaml" ? "yaml" : "md");
  }, [currentProject, yamlFile, mdPath, navigate]);

  const selectMarkdownsDir = () => {
    if (!browserPath) return;
    setMdPath(browserPath);
    setMdSuggested(false);
    setBrowserTarget(null);
    if (!title && !dirEdited) {
      const leaf = browserPath.replace(/[\\/]$/, "").split(/[\\/]/).pop() || "";
      setTitle(leaf);
  ...
  const selectYamlFile = (filename: string) => {
    ...
    setBrowserTarget(null);
    if (!mdPath) {
      const suggestion = joinPath(parentDir(fullPath), "markdowns");
      setMdPath(suggestion);
      setMdSuggested(true);
    }
    if (!title && !dirEdited) {
  ...
  const handleMdPathChange = (val: string) => {
    setMdPath(val);
    setMdSuggested(false);
    setError("");
  };
        ...
        <div style={{ background: "#fff", borderRadius: 8, minWidth: 480, maxWidth: 600, width: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", display: "flex", flexDirection: "column", maxHeight: "80vh" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {browserPath && (
                <button onClick={() => { setBrowserPath(""); setBrowserDirs([]); setBrowserFiles([]); setBrowserParent(null); }}
                  style={{ padding: "2px 6px", border: "1px solid #ccc", borderRadius: 4, background: "#f5f5f5", cursor: "pointer", fontSize: 11, color: "#999", flexShrink: 0 }}>✕</button>
              )}
            </div>
          </div>
          <div style={{ overflowY: "auto", flex: 1, padding: "2px 0" }}>
            {browserDirs.map(d => {
              ...
              return (
                <div key={d} onClick={() => navigate(d, ext)}
          ...
          <div style={{ padding: "10px 20px", borderTop: "1px solid #e8e8e8", display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button onClick={() => setBrowserTarget(null)} style={{ padding: "6px 16px", border: "1px solid #ccc", borderRadius: 4, background: "#f5f5f5", cursor: "pointer", fontSize: 13 }}>Back</button>
            {!isYaml && (
              <button onClick={selectMarkdownsDir} disabled={!browserPath}
                style={{ padding: "6px 16px", border: "none", borderRadius: 4, background: browserPath ? "#1a6fa8" : "#a0c4e8", color: "#fff", cursor: browserPath ? "pointer" : "default", fontSize: 13, fontWeight: 600 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 8, minWidth: 480, maxWidth: 560, width: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 3 }}>
                ...
                {mdSuggested && <span style={{ color: "#aaa", marginLeft: 6 }}>— suggested, click to confirm or browse to change</span>}
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <input value={mdPath} onChange={e => handleMdPathChange(e.target.value)}
                  placeholder="Browse to select a directory…"
                  style={{
            ...
            <div>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 3 }}>
                YAML file <span style={{ color: "#aaa" }}>(optional — browse to an existing .yaml/.yml)</span>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
              ...
              {yamlFile && (
                <div style={{ fontSize: 11, color: "#888", marginTop: 3 }}>
                  PiTH will use this file as the project's hierarchy. Supports PiTH, MkDocs, or custom YAML.
                </div>
              )}
            </div>
          </div>
        ...
        <div style={{ padding: "12px 20px", display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button onClick={handleSubmit} disabled={!canSubmit}
            style={{ padding: "6px 16px", border: "none", borderRadius: 4, background: canSubmit ? "#1a6fa8" : "#a0c4e8", color: "#fff", cursor: canSubmit ? "pointer" : "default", fontSize: 13, fontWeight: 600 }}>
            Create
          </button>
        </div>
      </div>
